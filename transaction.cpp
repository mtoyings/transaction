...
{
  double acb{1};
  double acb_per_share{1};
  unsigned int share_balance{1};
  double cgl{};
  while (p_head != nullptr)
  {
    Transaction *p_runthrough{p_head};
    //p_head = p_head->get_next();
    while (p_runthrough->get_next() != nullptr)
    {
      p_runthrough = p_head;
      p_head = p_head->get_next();
      ...
      { //true == Buy
        acb += p_runthrough->get_amount();
        share_balance += p_runthrough->get_shares();
        acb_per_share = acb / share_balance;
        cgl += p_runthrough->get_amount() - (p_runthrough->get_shares() * acb_per_share);
        p_runthrough->Transaction::set_acb(acb);
        p_runthrough->Transaction::set_acb_per_share(acb_per_share);
        p_runthrough->Transaction::set_share_balance(share_balance);
        p_runthrough->Transaction::set_cgl(cgl);
      }
      else if (p_runthrough->get_trans_type() == false)
      { //fasle == Sell
        acb = (p_runthrough->get_shares()) - acb_per_share;
...
double History::compute_cgl(unsigned int year) //FINAL SUBMISSION
{
  ...
  {
    Transaction *p_runthrough{p_head};
    while (p_runthrough->get_next() != 0)
    {

      if (p_runthrough->Transaction::get_year() == year)
      {
        return p_runthrough->Transaction::get_cgl();
      }
      p_runthrough = p_runthrough->get_next();
    }
  }
